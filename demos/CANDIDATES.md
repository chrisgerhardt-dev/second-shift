...
Five Clone / Refresh / Reimagine hubs for internal review, plus the Interim Execs client hub. No outreach. Do not email prospects.
...
| interimexecs | https://interimexecs.com | WordPress / WP Engine | Home + inner pages |
...
Interim Execs client hub (`demos/interimexecs/`) uses the **same locked play**. Current opens `https://www.interimexecs.com/` (canonical `https://interimexecs.com`). Webflow Clone stand-in is `interimexecs-refresh.webflow.io` (`choices.clone.webflowPreview.ready: false` — Staging soon; host was a 404 on 2026-09-09). Do not iframe webflow.io. Redesign ladder is hidden. DEMO / review-only. Talk first only — no Stripe. Public pages use `hello@secondshift.care` only.
